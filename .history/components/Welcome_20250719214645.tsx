...
function NeuralNetwork({ mouse, onNodeHover }) {
  // 6 nodes in a circle, lines between them, all rotate with mouse
  const group = useRef();
    ...
    <group ref={group}>
      ...
      {nodes.map((a, i) => nodes.map((b, j) => i < j && (
        <line key={i + '-' + j}>
          <bufferGeometry attach="geometry">
            <bufferAttribute attachObject={["attributes", "position"]} count={2} array={new Float32Array([...a, ...b])} itemSize={3} />
          </bufferGeometry>
      ...
      {nodes.map((pos, i) => (
        <mesh
          ...
          position={pos}
          onPointerOver={e => { setHovered(i); onNodeHover && onNodeHover(i); }}
          onPointerOut={e => setHovered(-1)}
        >
...
function InteractiveHero3D() {
  ...
  // Node hover handler (could show tooltip or effect)
  function handleNodeHover(i) {
    // For future: show tooltip or highlight
  }
  return (
    <Canvas camera={{ position: [0, 0, 8], fov: 60 }} dpr={[1, 2]} onPointerMove={handlePointerMove} style={{ cursor: 'pointer' }}>

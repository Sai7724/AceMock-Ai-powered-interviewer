...
const SectionTitle = ({ children, subtitle }: { children: React.ReactNode, subtitle?: string }) => (
  <div className="text-center mb-20 sm:mb-28">
    <h2 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold text-slate-100 font-sans tracking-tight leading-tight">
      {children}
    </h2>
    {subtitle && <p className="mt-8 text-2xl text-slate-400 max-w-3xl mx-auto font-sans leading-relaxed">{subtitle}</p>}
  </div>
);
...
function NeuralMesh3D() {
  // Mesh parameters
  const rows = 7;
  const cols = 12;
  const nodeSpacing = 0.6;
  const curveAmount = 2.2; // Controls the mesh's curvature
  const group = useRef<THREE.Group>(null);
  const [hovered, setHovered] = useState<number | null>(null);
  const mouse = useRef<{ x: number; y: number }>({ x: 0, y: 0 });

  // Generate node positions in a curved grid
  const nodes = useMemo(() => {
    const arr: [number, number, number][] = [];
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        // Curve the mesh in z for depth
        const x = (c - cols / 2) * nodeSpacing;
        const y = (r - rows / 2) * nodeSpacing;
        const z = Math.sin((c / (cols - 1)) * Math.PI) * curveAmount + Math.sin((r / (rows - 1)) * Math.PI) * curveAmount * 0.5;
        arr.push([x, y, z]);
      }
    }
    return arr;
  }, [rows, cols, nodeSpacing, curveAmount]);

  // Generate edges (lines between adjacent nodes)
  const edges = useMemo(() => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (c < cols - 1) arr.push([idx, idx + 1]);
        // Connect to bottom neighbor
        if (r < rows - 1) arr.push([idx, idx + cols]);
      }
    }
    return arr;
  }, [rows, cols]);

  // Animate rotation and node glow
  useFrame(({ clock }) => {
    if (group.current) {
    ...
    <group ref={group} onPointerMove={handlePointerMove}>
      ...
      {nodes.map(([x, y, z], i) => (
        <mesh
          ...
          scale={hovered === i ? 1.3 : 1 + Math.sin(Date.now() * 0.002 + i) * 0.08}
        >
          <sphereGeometry args={[0.13, 24, 24]} />
          <meshStandardMaterial
            color="#e0f2fe"
            emissive={hovered === i ? "#faffb0" : "#67e8f9"}
            emissiveIntensity={hovered === i ? 1.2 : 0.7}
            transparent
            opacity={0.95}
          />
        </mesh>
...
function InteractiveHero3D() {
  return (
    <Canvas camera={{ position: [0, 0, 10], fov: 60 }} dpr={[1, 2]} style={{ cursor: 'pointer' }}>
      <ambientLight intensity={0.8} />
      <directionalLight position={[5, 5, 5]} intensity={1.3} />
      <NeuralMesh3D />
    </Canvas>
...
function Hero({ onStart }: WelcomeProps) {
  return (
    <div id="home" className="relative text-center py-36 sm:py-48 lg:py-56 overflow-hidden min-h-[800px] flex items-center justify-center font-sans">
      <div className="absolute inset-0 w-full h-full z-0">
        <InteractiveHero3D />
      </div>
      <div className="relative z-10 max-w-4xl mx-auto px-4">
        <h1 className="text-6xl sm:text-7xl lg:text-8xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-50 to-cyan-400 mb-10 animate-fade-in-up font-sans tracking-tight leading-tight">
          Unlock Your Career Potential
        </h1>
        <p className="max-w-4xl mx-auto text-2xl sm:text-3xl text-slate-300 mb-16 animate-fade-in-up delay-200 font-sans leading-relaxed">

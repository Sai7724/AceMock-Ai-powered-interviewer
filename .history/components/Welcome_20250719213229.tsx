...
const Section = ({ children, className = '', id }: SectionProps) => (
  <section id={id} className={`py-20 sm:py-28 lg:py-32 ${className} font-grotesk`}>
...
const SectionTitle = ({ children, subtitle }: { children: React.ReactNode, subtitle?: string }) => (
  <div className="text-center mb-16 sm:mb-20">
    <h2 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-100 font-grotesk tracking-tight leading-tight">
      {children}
    </h2>
    {subtitle && <p className="mt-6 text-xl text-slate-400 max-w-2xl mx-auto font-grotesk leading-relaxed">{subtitle}</p>}
  </div>
);
...
const Hero = ({ onStart }: { onStart: () => void }) => (
  <div id="home" className="relative text-center py-28 sm:py-36 lg:py-44 overflow-hidden min-h-[700px] flex items-center justify-center font-grotesk">
    <Hero3DBackground />
    <div className="relative z-10 max-w-4xl mx-auto px-4">
      <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-50 to-cyan-400 mb-6 animate-fade-in-up font-grotesk tracking-tight leading-tight">
        Unlock Your Career Potential
      </h1>
      <p className="max-w-3xl mx-auto text-xl sm:text-2xl text-slate-300 mb-12 animate-fade-in-up delay-200 font-grotesk leading-relaxed">
      ...
      <div className="animate-fade-in-up delay-400">
        <button
          onClick={onStart}
          className="bg-cyan-500 text-white font-bold py-4 px-12 rounded-xl shadow-xl shadow-cyan-500/20 transform hover:scale-105 transition-all duration-300 ease-in-out text-xl hover:bg-cyan-400 font-grotesk"
        >

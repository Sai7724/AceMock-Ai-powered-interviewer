...
export default function Navbar({ onReset, showControls, onShowProfile, onShowLogin }: NavbarProps & { onShowProfile?: () => void; onShowLogin?: () => void; }) {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [active, setActive] = useState('Home');
  const { user, signOut } = useAuth ? useAuth() : { user: null, signOut: () => {} };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 font-sans">
      <nav className="w-full max-w-7xl mx-auto px-8 sm:px-16 lg:px-24">
        <div className="flex items-center justify-between h-28">
          <div className="flex items-center space-x-6 cursor-pointer select-none">
            <Logo />
            <span className="text-4xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-100 to-cyan-400 tracking-tight font-sans">AceMock</span>
          </div>
          <div className="hidden md:flex items-center space-x-12">
            {navLinks.map(link => (
              <a
                ...
                className={`relative font-semibold px-2 py-1 transition-colors duration-200 text-lg ${active === link.label ? 'text-cyan-400' : 'text-slate-200 hover:text-cyan-300'}`}
              >
                {link.label}
                {active === link.label && (
      ...
      <div
        ...
        style={{ zIndex: 49 }}
      >
        <div className="flex flex-col items-center py-8 space-y-6 animate-fade-in-up">
          {navLinks.map(link => (
            <a
              key={link.label}
              href={link.href}

...
// Ensure we test the real geminiService implementation (not the global mock from test setup)
vi.unmock('../services/geminiService');

// Mock the GoogleGenAI implementation before importing the service
...
describe('Gemini Service', () => {
  // Note: we do NOT reset mocks here because the Gemini service creates a single
  // GoogleGenAI instance at module initialization time. Resetting mocks would
  // clear the instantiated mock object, breaking access to its methods.
  ...
  describe('analyzeSelfIntroduction', () => {
    it('should analyze self introduction and return feedback', async () => {
      ...
      ai.models.generateContent = vi.fn().mockResolvedValue(mockResponse);

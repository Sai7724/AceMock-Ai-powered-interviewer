...
function getApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error('VITE_GEMINI_API_KEY environment variable is not set. Please create a .env.local file in the project root.');
  }
...
} satisfies Pick<GoogleGenAI, 'models'>;

function parseModelJson<T>(text: string | undefined, operation: string): T {
  if (!text || !text.trim()) {
  ...
  try {
    ...
    console.error(`${operation} returned invalid JSON:`, text);
    throw error;
  }
}

function normalizeHrCategory(category: string): HRQuestionCategory {
  switch (category.toLowerCase()) {
    ...
    case 'teamwork':
    case 'leadership':
      return category.toLowerCase() as HRQuestionCategory;
    default:
      return 'behavioral';
  }
}
...
export async function analyzeSelfIntroduction(introduction: string): Promise<SelfIntroductionFeedback> {
  ...
  const jsonResponse = parseModelJson<SelfIntroductionFeedback>(response.text, 'Self-introduction analysis');
...
export async function generateAptitudeQuestions(count: number): Promise<AptitudeQuestion[]> {
    ...
    const jsonResponse = parseModelJson<AptitudeQuestion[]>(response.text, 'Aptitude question generation');
...
export async function evaluateAptitudePerformance(questions: AptitudeQuestion[], userAnswers: string[]): Promise<AptitudeFeedback> {
    ...
    const feedbackPart = parseModelJson<Omit<AptitudeFeedback, 'correctCount' | 'totalQuestions' | 'detailedResults'>>(response.text, 'Aptitude evaluation');
...
export async function generateTechnicalQuestions(language: string): Promise<string[]> {
    ...
    const jsonResponse = parseModelJson<string[]>(response.text, 'Technical question generation');
...
export async function evaluateTechnicalAnswers(questions: string[], answers: string[], language: string): Promise<TechnicalQAFeedback> {
    ...
    const jsonResponse = parseModelJson<TechnicalQAFeedback>(response.text, 'Technical answer evaluation');
...
export async function generateCodingChallenge(language: string): Promise<{ title: string; description: string; defaultCode: string; }> {
    ...
    const parsed = parseModelJson<{ title: string; description: string; defaultCode: string; }>(response.text, 'Coding challenge generation');
...
export async function evaluateCode(problem: string, language: string, code: string): Promise<CodingFeedback> {
    ...
    const jsonResponse = parseModelJson<CodingFeedback>(response.text, 'Code evaluation');
    return jsonResponse as CodingFeedback;
}

export async function generateHRQuestions(): Promise<{ question: string; category: HRQuestionCategory; }[]> {
    ...
    const jsonResponse = parseModelJson<Array<{ question: string; category: string }>>(response.text, 'HR question generation');
    return jsonResponse.map((item) => ({
      question: item.question,
      category: normalizeHrCategory(item.category),
    }));
}

export async function evaluateHRResponse(questions: { question: string; category: HRQuestionCategory; }[], responses: string[]): Promise<HRFeedback> {
    ...
    const jsonResponse = parseModelJson<HRFeedback>(response.text, 'HR response evaluation');
